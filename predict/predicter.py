...
def event_clue_predict(text, flag_id):
    ...
    token_ids = torch.tensor(token_ids).to(config.device)
    masks = torch.tensor(masks).to(config.device)

    data_flag = flags[flag_id]
    model = ClassifierWithBert4Layer(config=config, flag=data_flag).to(config.device)

    if data_flag == 'score':
        model.load_state_dict(torch.load(r"../data/saved_dict/10_22_train/10_21_data_event_score10-22_18.00.ckpt"))
    elif data_flag == 'scale':
        model.load_state_dict(torch.load(r"../data/saved_dict/10_22_train/10_21_data_event_scale10-22_18.00.ckpt"))
    elif data_flag == 'influence':
        model.load_state_dict(torch.load(r"../data/saved_dict/10_22_train/10_21_data_event_influence10-22_18.00.ckpt"))
    elif data_flag == 'strength':
        model.load_state_dict(torch.load(r"../data/saved_dict/10_22_train/10_21_data_event_strength10-23_10.01.ckpt"))
    elif data_flag == 'degree':
        model.load_state_dict(torch.load(r"../data/saved_dict/10_22_train/10_21_data_event_degree10-23_10.01.ckpt"))
    else:
